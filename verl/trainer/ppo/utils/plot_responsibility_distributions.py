...
Usage (standalone):
    python plot_responsibility_distributions.py --json_path path/to/step_100_advantages_final.json --output_path output.pdf

Usage (from training code):
    from visuals.plot_responsibility_distributions import plot_responsibility_distributions
    fig = plot_responsibility_distributions(json_data_or_path, save_path=None)
"""
...
def load_responsibility_data(json_path_or_data: Union[str, Path, Dict]) -> Dict[str, Any]:
    """
    Load responsibility data from JSON file or dict.

    Args:
        json_path_or_data: Path to JSON file or dict with responsibility data

    Returns:
        Dictionary with responsibility data
    """
...
def plot_responsibility_distributions(
    ...
    *,
    ...
    """
    Create box and whisker plot with individual points for responsibility distributions per task.
    ...
    """
    # Load data
    data = load_responsibility_data(json_path_or_data)
    ...
    for task_name in sorted_tasks:
        ...
        responsibilities = task_info.get("responsibilities", [])

        # Skip tasks with no responsibilities or all zeros
        if not responsibilities or all(r == 0.0 for r in responsibilities):
            continue

        # Add box plot for the task (without showing individual points in the box)
        box_traces.append(go.Box(
            y=responsibilities,
            x=[task_name] * len(responsibilities),
            name=task_name,
    ...
    if not box_traces:
        print("No responsibility data found in JSON (all responsibilities are 0.0)")
        # Create empty figure with message
        fig = go.Figure()
        fig.update_layout(
            title="No Responsibility Data Available",
            annotations=[
                dict(
                    text="No responsibility data found (all values are 0.0)",
                    xref="paper",
                    yref="paper",
                    x=0.5,
                    y=0.5,
    ...
    # Set title
    if title is None:
        title = f"Responsibility Distributions per Task (Step {step}, {advantage_type})"
    ...
    if save_path:
        ...
        print(f"Saved responsibility plot to {save_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to JSON file with responsibility data"
    )
    parser.add_argument(
        "--output_path",
        type=str,
    ...
    fig = plot_responsibility_distributions(
            args.json_path,
