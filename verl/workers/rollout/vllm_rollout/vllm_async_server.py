...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        if max_tokens < 1:
            # Diagnose why we ran out of room before clamping: count image/video pad
            # tokens vs text tokens (a flood of these signals a prompt-length problem).
            n_image_tokens = 0
            n_video_tokens = 0
            ...
            logger.warning(
                f"max_tokens computed as {max_tokens} (<1) before clamping to 1, so effectively no new "
                f"tokens will be generated. Prompt length ({len(prompt_ids)}) is close to or exceeds the "
                f"model's maximum context length ({self.config.max_model_len}). Response length: "
                f"{self.config.response_length}, prompt length config: {self.config.prompt_length}. "
