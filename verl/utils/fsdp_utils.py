...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    if min_num_params > 0:
        ...
        # `fsdp_transformer_layer_cls_to_wrap` may list names that don't resolve to a
        # concrete class -- some HF releases expose forward-compat names in
        # `_no_split_modules` (e.g. Qwen3.5 lists `Qwen3_5TextDecoderLayer` alongside the
        # real `Qwen3_5DecoderLayer`). Names we can resolve are wrapped by class; for the
        # rest we fall back to a lambda matching on runtime class name, so a single unknown
        # name neither breaks the wrap policy nor leaves matching layers unwrapped.
