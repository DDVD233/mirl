...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    async def _create_teacher_model_manager(self):
        ...
        if is_distillation_enabled(self.config.get("distillation")):
            ...
            # Resolve the teacher resource pool, supporting three modes:
            #   1. Co-located teacher already registered in the trainer's pool manager.
            #   2. Dedicated teacher pool sized from the distillation config (upstream).
            #   3. External (off-cluster) teacher served over HTTP -> no local GPUs.
            if Role.TeacherModel in self.resource_pool_manager.mapping:
                ...
                if n_gpus > 0:
                    ...
                    resource_pool_spec = {"teacher_pool": [n_gpus] * nnodes}
