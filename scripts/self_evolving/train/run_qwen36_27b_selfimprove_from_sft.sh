#!/usr/bin/env bash
# Self-evolving RL run INITIALISED FROM THE SFT CHECKPOINT.
#
# Same self-evolving DATA pipeline as run_qwen36_27b_evolve_reward.sh with
# EVOLVE_ENABLE=False (i.e. byte-identical to run_qwen36_27b_selfimprove_kl.sh):
#   * training questions come from the gen-server proposer (SelfEvolvingDataset,
#     :8006) — self-evolving data is ON,
#   * the per-sample reward is the FIXED composite (judge + embed + bleu + ...) —
#     reward evolution is OFF,
#   * the ACTOR is initialised from a merged SFT checkpoint (ACTOR_MODEL_PATH)
#     instead of the base model. The judge / teacher / gen-server LLM stay the
#     base Qwen3.6-27B on SERVER 5 (http://point.dd.works:18184) — the judge does
#     NOT load the SFT checkpoint.
#
# Prereqs (start FIRST, same node):
#   1. merge the SFT ckpt -> HF weights:
#        python -m verl.model_merger merge --backend fsdp \
#          --local_dir <sft_ckpt>/global_step_N/actor --target_dir $ACTOR_MODEL_PATH
#   2. gen server on :8006 (scripts/self_evolving/serve/start_generation_server.sh
#      with API_BASE=:18184, MILVUS_COLLECTION=medical_knowledge_v2, PORT=8006).
set -xeuo pipefail
...
# Actor init: merged SFT checkpoint (HF format). REQUIRED — override at launch.
ACTOR_MODEL_PATH="${ACTOR_MODEL_PATH:?set ACTOR_MODEL_PATH to the merged SFT HF dir}"

# Reward evolution OFF — fixed composite reward (== the validation scoring path).
EVOLVE_ENABLE="${EVOLVE_ENABLE:-False}"
...
/usr/local/bin/python -m verl.trainer.main_ppo \
    ...
    +trainer.validation_data_dir="/scratch/sheng/self_evolving/logs_selfimprove_from_sft/val_generations/$EXP" \
