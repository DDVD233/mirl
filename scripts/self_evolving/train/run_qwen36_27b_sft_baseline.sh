#!/usr/bin/env bash
# SFT baseline: Qwen3.6-27B trained with supervised CE loss (sft_loss) on the
# REAL mimiciv_rare train set — train input -> train output only. NO
# self-evolving generation server, NO teacher traces, NO reward evolution, NO
# feedback loop. The supervised target is the dataset's own ground-truth answer
# wrapped as \boxed{CODE: Diagnosis} (see static_sft_dataset.StaticSFTDataset).
#
# This is the SFT control for run_qwen36_27b_baseline.sh (GRPO) and the
# self-evolving runs. It uses the SAME validation set (test.jsonl) and is scored
...
EXP="${EXP:-mimiciv_rare_qwen36_27b_sft_baseline}"
...
/usr/local/bin/python -m verl.trainer.main_sft_evolving \
    ...
    data.custom_cls.path=scripts/self_evolving/static_sft_dataset.py \
    data.custom_cls.name=StaticSFTDataset \
    ...
    +trainer.validation_data_dir="/scratch/sheng/self_evolving/logs_sft_baseline/val_generations/$EXP" \
