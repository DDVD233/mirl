...
#
# DEADLOCK NOTE (gpt-5.3 judge is slow + remote): while the driver judges, the vLLM
# rollout TP workers sit idle; past ~7 min idle their NCCL collective spins (all 4
# GPUs -> 99%, log frozen, never recovers). The fast local Qwen judge (self-improve
# run) finishes each phase well under that, so it never trips; gpt-5.3 does. verl's
# `sync` rollout (which would decouple this) has been REMOVED in this verl version, so
# the mitigation is to keep every judge phase short:
#   - val_max_samples=512 (not full 2452): each val judge phase ~3 min < ~7 min thresh.
#   - REWARD_JUDGE_CONCURRENCY=48 at launch (4x the default 16) keeps per-step reward
#     judging (~512x4 calls) to ~3 min as well.
#   - per-step training reward stays FULL (all judges); only the in-training val is
#     subsampled. Get the authoritative FULL-2452 baseline via a standalone rejudge.
# Concurrency 96+ / full val would also work numerically but risks the shared TRAPI
