#!/usr/bin/env bash
# Multi-agent self-evolving training on PMC-VQA.
#
# Architecture:
#   - Milvus knowledge base: http://mib.media.mit.edu:19531 (remote, reached from vps3)
#   - vLLM chat API (proposer/generator/validator/judge): spun up on GPUs 0,1
#   - vLLM embedding API (for Milvus queries): spun up on GPU 1 (shared) or separate
#   - Training: GPUs 2,3 with verl
#
# Prerequisites (on vps3):
#   - conda env `cu129` (or `cu128`) with vllm for the servers
...
#   - PMC-VQA preprocessed: /scratch/self_evolving_datasets/pmc_vqa_masked/{train,test}.jsonl
#
# Environment variables:
#   API_BASE       - chat/LLM API URL (default: http://localhost:8000/v1)
#   EMBED_API_BASE - embedding API URL (default: http://localhost:8001/v1)
#   MILVUS_URI     - Milvus server URI
#   MILVUS_TOKEN   - Milvus auth token
#   MODEL_NAME     - chat model (default: Qwen/Qwen3-VL-8B-Instruct)
...
if [ ! -f "$DATA_DIR/train.jsonl" ]; then
    echo "Preprocess PMC-VQA first: python scripts/self_evolving/preprocess_pmcvqa.py"
    exit 1
fi
...
python3 -m verl.trainer.main_ppo \
    ...
    +data.self_evolving.milvus_top_k=3 \
