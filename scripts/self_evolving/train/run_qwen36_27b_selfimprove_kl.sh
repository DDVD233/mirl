#!/usr/bin/env bash
# Re-run of the qwen36_27b self-improve run (the 0.350 best, replicated from
# mimiciv_rare_qwen36_27b_selfimprove_20260613_183109) WITH a small KL-to-reference
# loss re-enabled for stability. The original disabled it (use_kl_loss=False) to go
# faster; training was a bit unstable, so we add it back small:
#     actor.use_kl_loss=True  kl_loss_coef=0.001  kl_loss_type=low_var_kl
# This costs an extra reference-policy forward pass per step (slower) but damps the
# policy from drifting too far. Everything else is byte-identical to the replicate.
#
# Trainer on SERVER 4 (4 GPUs). Teacher (same Qwen3.6-27B) on SERVER 5,
# http://point.dd.works:18184, used as the gen-server LLM + reward self-judge.
# Gen server on server 4 :8006 (start_gen_server_mimic_s1.sh BEFORE this).
# val = mimiciv_rare/test.jsonl, val_max_samples=-1 (full per-ICD-category eval).
set -xeuo pipefail
...
EXP="${EXP:-mimiciv_rare_qwen36_27b_selfimprove_kl}"
...
# B200 / vLLM 0.22rc robustness: the async rollout engine auto-selected FlashInfer's
# TRTLLM prefill kernel, which hung >5min on a long multimodal prompt at step 17 and
# killed the engine (EngineDeadError: "RPC call to sample_tokens timed out"). Give the
# per-call execute-model RPC a generous ceiling so one slow sample can't kill the engine.
...
/usr/local/bin/python -m verl.trainer.main_ppo \
    ...
    +trainer.validation_data_dir="/scratch/sheng/self_evolving/logs_si_qwen36_kl/val_generations/$EXP" \
