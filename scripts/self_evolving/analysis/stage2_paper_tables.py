...
def main():
    ...
    # Captions follow the wording of the 2026-09-11 ML4H-synced draft (method renamed RRI).
    cap_main = (r"HealthBench Professional accuracy using the official length-adjusted rubric score on all 525 tasks "
                r"by category, by difficulty, and overall. Each group of rows is one training setting, sharing one "
                r"base model and grading protocol; within a setting, \emph{Fixed prompt} and \emph{RRI} share "
                r"the identical RL recipe, solver tools, grader, and data budget, and differ only in whether the "
                r"reward is held fixed or allowed to evolve. Bold marks the better trained row per column.")
    open(os.path.join(T, "hbpro_main.tex"), "w").write(provenance + render_main(table, PRIM, cap_main, "tab:main"))
    cap_raw = (r"Unadjusted rubric accuracy as the fraction of rubric points earned with the length term omitted "
    ...
    cap_abl = (r"Component ablation of RRI on Qwen3.5-9B under the GPT grader. The upper group runs with retrieval "
    ...
    cap_sp = (r"Per-specialty accuracy using the official length-adjusted score for the fixed-prompt and RRI rows "
