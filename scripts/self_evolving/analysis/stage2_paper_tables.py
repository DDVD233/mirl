...
def main():
    ...
    cap_main = (r"HealthBench Professional accuracy (official length-adjusted rubric score, all 525 tasks) "
                r"by category, by difficulty, and overall. Each block is one base model and grading protocol; "
                r"within a block, \emph{Fixed prompt} and \emph{SER (ours)} share the identical RL recipe, solver tools, "
                r"grader, and data budget, and differ only in whether the reward is held fixed or allowed to evolve. "
    ...
    cap_raw = (r"Companion to Table~\ref{tab:main}: unadjusted rubric accuracy (fraction of rubric points earned, "
               r"no length term) at the same evaluations.")
    open(os.path.join(T, "hbpro_main_raw.tex"), "w").write(provenance + render_main(table, SEC, cap_raw, "tab:main-raw"))
    cap_abl = (r"Component ablation of SER on Qwen3.5-9B under the GPT grader. Top: without retrieval, adding meta-prompt "
               r"evolution and then the admission-time adversary probe to a calibrated fixed prompt. Bottom: with retrieval "
               r"and web search, from the untuned fixed prompt through the calibrated prompt, evolution with the "
               r"admission-time adversary probe, and the full on-policy hack-then-patch loop. Official length-adjusted "
    ...
    cap_sp = (r"Per-specialty accuracy (official length-adjusted score) for the fixed-prompt and SER rows "
              r"of each block in Table~\ref{tab:main}. $n$ is the number of validation tasks in the specialty.")
