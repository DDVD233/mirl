...
# can never come up as the wrong arm. The arm serves its own summarizer on the last
# GPU of the allocation (SUMM_BASE local) and trains the policy on the other three.
#
...
set -euo pipefail
ARM=23 EXP_SUFFIX="${EXP_SUFFIX:-_aicr}" N_GPUS="${N_GPUS:-3}" FROZEN_GPU="${FROZEN_GPU:-3}" \
