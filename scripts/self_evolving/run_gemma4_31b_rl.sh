#!/usr/bin/env bash
# Gemma-4-31B-it GRPO RL on server1 (4x B200). run_qwen36_27b_full.sh GRPO recipe,
# but adapted for Gemma: head_dim=256 -> sdpa + use_remove_padding=False, and a
# FIXED small micro-batch (NOT dynamic-bsz: 16k-token packs OOM the sdpa forward on
# 31B). Rollout mem kept low (0.3) so the offloaded actor forward has headroom.
# Teacher via TRAPI: gpt-5.5 gen (:8004) + gpt-5.1-chat judge. Multimodal val.
set -xeuo pipefail
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-mimiciv_rare_gemma4_31b_rl_gpt51judge_${RUN_TS}}"
...
VALIDATION_DATA_DIR="/scratch/sheng/self_evolving/logs/val_generations/$EXPERIMENT_NAME"
cd "$REPO_ROOT"
"$PYTHON_BIN" -m verl.trainer.main_ppo \
    ...
    "$@" 2>&1 | tee "/root/gemma31b_rl_${EXPERIMENT_NAME}.log"
