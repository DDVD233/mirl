#!/usr/bin/env bash
# Gemma-4-31B-it SELF-IMPROVEMENT GRPO on an 8x B200 node. The SAME model is the
# gen-server backend + reward judge (local vLLM :8100 on GPUs 0-3, via
# serve_gemma4_31b_vllm.sh + serve_gen_gemma4_31b.sh) AND the RL student (GPUs 4-7).
# Settings mirror run_gemma4_31b_rl.sh: Gemma head_dim=256 -> sdpa + use_remove_padding=False;
# fixed micro-batch=1 (dynamic-bsz OOMs the 31B sdpa forward); rollout mem 0.3; B200 FA2.
# Validation reads the real multimodal mimiciv_rare/test.jsonl (RLHFDataset).
set -xeuo pipefail
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-mimiciv_rare_gemma4_31b_selfimprove_${RUN_TS}}"
