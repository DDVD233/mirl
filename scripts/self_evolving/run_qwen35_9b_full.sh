#!/usr/bin/env bash
# Full Qwen3.5-9B training on node3400 (4xH200) with self-evolving generation
# server. Chat server / judge stays at Qwen3.6-27B on node2500.
#
# Smaller actor than run_qwen36_27b_full.sh so we can bump rollout headroom
# (TP=1, larger micro-batch, higher gpu_memory_utilization) and avoid the OOM
# the 27B variant hit.
...
export EXPERIMENT_NAME="${EXPERIMENT_NAME:-mimiciv_rare_qwen35_9b}"
