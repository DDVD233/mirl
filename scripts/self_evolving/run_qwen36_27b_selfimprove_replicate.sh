#!/usr/bin/env bash
# Faithful re-run of mimiciv_rare_qwen36_27b_selfimprove_20260613_183109, from the
# exact wandb run config. Trainer on server 1 (4 GPUs); teacher (same 27B) served
# on server 5 at point.dd.works:18184 and used as the gen-server LLM + reward
# self-judge; gen server on server 1 :8006. val = re-tagged mimiciv_rare/test.jsonl
# (per-ICD-category metrics auto-emitted). val_max_samples=-1 (full eval).
set -xeuo pipefail
...
EXP="${EXP:-mimiciv_rare_qwen36_27b_selfimprove_replicate}"
...
/usr/local/bin/python -m verl.trainer.main_ppo \
    ...
    +trainer.validation_data_dir="/scratch/sheng/self_evolving/logs_si_qwen36_replicate/val_generations/$EXP" \
