...
BASE_DIR=/scratch/keane/human_behaviour/22apr_gpg
...
for CKPT_DIR in $(ls -d "${BASE_DIR}"/global_step_* 2>/dev/null | sort -t_ -k3 -n); do
    echo "=== Running validation for: $CKPT_DIR ==="
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='22apr_gpg' \
        ...
        trainer.validation_data_dir="${BASE_DIR}" \
        trainer.test_freq=99999 \
        trainer.total_epochs=5 \
        trainer.advantage_save_dir="${BASE_DIR}/advantages" \
        trainer.advantage_plot_freq=15 $@ \
        trainer.default_local_dir="${BASE_DIR}"
done