...
# And also the micro batch size per gpu to 8
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='19apr_grpo' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=10 \
        trainer.val_before_train=False \
        trainer.val_only=False \
        trainer.validation_data_dir=/scratch/keane/hb_atlas_models/19apr_grpo \
        trainer.test_freq=99999 \
        trainer.total_epochs=5 \
        trainer.advantage_save_dir=/scratch/keane/hb_atlas_models/19apr_grpo/advantages \
        trainer.advantage_plot_freq=15 $@ \
        trainer.default_local_dir=/scratch/keane/hb_atlas_models/19apr_grpo
