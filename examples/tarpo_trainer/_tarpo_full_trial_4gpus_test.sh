...
# originally prompt length, response length, max model len is 2048, 2048, 8192 
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='harpo_full_global_step_350_test' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=999999 \
        trainer.val_before_train=True \
        trainer.resume_mode=resume_path \
        trainer.resume_from_path=/scratch/keane/human_behaviour/HARPO_Hier_Ema_Rerun/global_step_350 \
        trainer.val_only=True \
        trainer.validation_data_dir=/scratch/keane/human_behaviour/HARPO_Hier_Ema_Rerun/global_step_350_test \
        trainer.test_freq=1 \
