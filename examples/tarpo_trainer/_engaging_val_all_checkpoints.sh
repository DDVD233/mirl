...
# ============================================
CHECKPOINT_FOLDER="/scratch/keane/human_behaviour/dapo_enhanced_harpo"
ALGO_NAME="harpo_full_v2_engaging"
MIN_CHECKPOINT_STEP=350  # Only evaluate checkpoints >= this step number (0 = evaluate all)
EXCLUDE_CHECKPOINTS="450 600"   # Space-separated list of checkpoint steps to skip (e.g., "300 450 600")
# ============================================
...
mapfile -t ckpt_lines < <(
  for d in "$CHECKPOINT_FOLDER"/global_step_*; do
    ...
    # exclusion filter
    if [[ -n "$EXCLUDE_CHECKPOINTS" && " $EXCLUDE_CHECKPOINTS " == *" $step "* ]]; then
      echo "Skipping $bn (step $step is in EXCLUDE_CHECKPOINTS)" >&2
      continue
    fi
