...
# originally prompt length, response length, max model len is 2048, 2048, 8192 
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='naive_density' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=25 \
        trainer.val_before_train=False \
        trainer.val_only=False \
        trainer.validation_data_dir=/scratch/keane/human_behaviour/naive_density \
        trainer.test_freq=50 \
        trainer.total_epochs=5 \
        trainer.advantage_save_dir=/scratch/keane/human_behaviour/naive_density/advantages \
