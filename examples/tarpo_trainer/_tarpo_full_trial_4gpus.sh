...
# originally prompt length, response length, max model len is 2048, 2048, 8192 
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        trainer.experiment_name='tarpo_iter_5_mixtures' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=50 \
        trainer.val_before_train=False \
        trainer.val_only=False \
        trainer.validation_data_dir=/scratch/keane/human_behaviour/tarpo_iter_5_mixtures \
        trainer.test_freq=50 \
        trainer.total_epochs=5 $@ \
        trainer.default_local_dir=/scratch/keane/human_behaviour/tarpo_iter_5_mixtures