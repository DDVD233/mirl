...
# originally prompt length, response length, max model len is 2048, 2048, 8192 
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        reward_model.reward_manager=hb_dapo \
        trainer.critic_warmup=0 \
        trainer.logger='["console","wandb"]' \
        trainer.project_name='rl_omni_heldout' \
        trainer.experiment_name='dapo_enhanced_harpo' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=50 \
        trainer.val_before_train=False \
        trainer.val_only=False \
        trainer.validation_data_dir=/scratch/keane/human_behaviour/dapo_enhanced_harpo \
        trainer.test_freq=150 \
        trainer.total_epochs=5 \
        trainer.advantage_save_dir=/scratch/keane/human_behaviour/dapo_enhanced_harpo/advantages \
