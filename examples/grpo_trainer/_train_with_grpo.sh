...
# originally prompt length, response length, max model len is 2048, 2048, 8192 
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        custom_reward_function.path=/home/keaneong/human-behavior/verl/examples/reward_function/human_behaviour_harpo.py \
        custom_reward_function.name=human_behaviour_compute_score_batch \
        reward_model.reward_manager=batch \
        trainer.critic_warmup=0 \
        trainer.logger='["console","wandb"]' \
        trainer.project_name='rl_omni_heldout' \
        trainer.experiment_name='harpo_iter_17_corrected_hierarchy_mixtures_densities_ema_original' \
        trainer.n_gpus_per_node=4 \
        trainer.nnodes=1 \
        trainer.save_freq=50 \
        trainer.val_before_train=False \
        trainer.val_only=False \
        trainer.validation_data_dir=/scratch/keane/human_behaviour/harpo_iter_17_corrected_hierarchy_mixtures_densities_ema_original \
        trainer.test_freq=50 \
        trainer.total_epochs=5 \
        trainer.advantage_save_dir=/scratch/keane/human_behaviour/harpo_iter_17_corrected_hierarchy_mixtures_densities_ema_original/advantages \
        trainer.advantage_plot_freq=15 $@ \
        trainer.default_local_dir=/scratch/keane/human_behaviour/harpo_iter_17_corrected_hierarchy_mixtures_densities_ema_original