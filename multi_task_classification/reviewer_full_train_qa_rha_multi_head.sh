#!/bin/bash
echo "Starting LORA + RHA training..."
export CUDA_VISIBLE_DEVICES="2,3"
export CUDA_LAUNCH_BLOCKING=1
export TORCH_USE_CUDA_DSA=1

# For videos, use this:
#   --use_rla_video \
#   --rla_stage joint \
#   --d_video_feat 210 \
#   --rla_hidden 128 \
#   --rla_p_moddrop_video 0.30
#
# For audio, use this:
#   --use_rla_audio \
#   --rla_stage residual_only \
#   --d_audio_feat 88 \
#   --rla_hidden 128 \
#   --rla_p_moddrop_audio 0.30
#
# Mean would be 1659
# meanstd would be 3318
# meanstdp25p75 would be 6636
#
# we have a few different options; base_only, residual_only, joint
#
# --load_checkpoint_path "/scratch/keane/human_behaviour/debug_rla/step_1998"
# for lora we put 1e-4
# but for rla we put 1e-3
#
# CONFIDENCE GAIN IS BASICALLY WHETHER OR NOT WE SCALE THE CONFIDENCE OF THE LOGITS THAT ARE FED INTO THE ADAPTERSS
#   --rla_video_use_ln \
#   --rla_audio_use_ln \
#   --rla_video_use_conf_gain \
#   --rla_video_conf_init_gain 3.0 \
#   --rla_audio_use_conf_gain \
#   --rla_audio_conf_init_gain 3.0 \
#   --rla_video_alpha_init 2.0 \
#   --rla_audio_alpha_init 2.0 \
#
# --load_checkpoint_path "/scratch/keane/human_behaviour/debug_rla/step_1998" \
#
# --label_map_path "/home/keaneong/human-behavior/verl/multi_task_classification/unified_feat_meld_label_map.json" \
# --rla_resume_diff_training_stage \
#
# MIMEQA:
# last_train_mimeqa_rha.jsonl
# last_test_mimeqa_rha.jsonl
#
# INTENTQA:
# last_train_intentqa_rha.jsonl
# last_test_intentqa_rha.jsonl
#
# SIQ2:
# last_train_siq2_rha.jsonl
# last_test_siq2_rha.jsonl

DATA_DIR="/scratch/keane/human_behaviour/human_behaviour_data"
BASE_SAVE_DIR="/scratch/keane/human_behaviour/reviewer_expts_rha_qa_multi_task_model"
BASE_LOAD_CKPT="/scratch/keane/human_behaviour/4_freeze_base_qa_multi_task_model/step_4578"
LABEL_MAP="/home/keaneong/human-behavior/verl/multi_task_classification/label_maps/unified_label_map_v6.json"

# Datasets to iterate over
# TASKS=("mimeqa" "intentqa" "siq2")
TASKS=("mimeqa")

for TASK in "${TASKS[@]}"; do
  echo "============================================================"
  echo "Starting run for task: ${TASK}"
  echo "============================================================"

  if [ "${TASK}" == "mimeqa" ]; then
    TRAIN_FILE="${DATA_DIR}/last_train_mimeqa_rha.jsonl"
    VAL_FILE="${DATA_DIR}/last_test_mimeqa_rha.jsonl"
    TEST_FILE="${DATA_DIR}/last_test_mimeqa_rha.jsonl"
  elif [ "${TASK}" == "intentqa" ]; then
    TRAIN_FILE="${DATA_DIR}/last_train_intentqa_rha.jsonl"
    VAL_FILE="${DATA_DIR}/last_test_intentqa_rha.jsonl"
    TEST_FILE="${DATA_DIR}/last_test_intentqa_rha.jsonl"
  elif [ "${TASK}" == "siq2" ]; then
    TRAIN_FILE="${DATA_DIR}/last_train_siq2_rha.jsonl"
    VAL_FILE="${DATA_DIR}/last_test_siq2_rha.jsonl"
    TEST_FILE="${DATA_DIR}/last_test_siq2_rha.jsonl"
  else
    echo "Unknown TASK: ${TASK}, skipping..."
    continue
  fi

  SAVE_DIR="${BASE_SAVE_DIR}/${TASK}"
  VAL_RESULT_DIR="${SAVE_DIR}/${TASK}_test_results"

  accelerate launch --config_file configs/accelerate_config_qwen.yaml train_qa_rha_multi_head.py \
    --rla_resume_diff_training_stage \
    --mode train \
    --training_strategy lora \
    --train_batch_size 2 \
    --val_batch_size 2 \
    --test_batch_size 2 \
    --lr 1e-4 \
    --hard_gamma 0.0 \
    --base_lr 1e-4 \
    --rla_lr  5e-4 \
    --epochs 1 \
    --train_file "${TRAIN_FILE}" \
    --val_file   "${VAL_FILE}" \
    --test_file  "${TEST_FILE}" \
    --label_map_path "${LABEL_MAP}" \
    --save_every_n_epochs 9999 \
    --save_every_n_steps 9999 \
    --save_checkpoint_dir "${SAVE_DIR}" \
    --validation_result_dir "${VAL_RESULT_DIR}" \
    --load_checkpoint_path "${BASE_LOAD_CKPT}" \
    --validate_every_n_epochs 1 \
    --validate_every_n_steps 999999 \
    --early_stopping_patience 99999 \
    --project "reviewer_ablate_raw_modalities" \
    --gradient_accumulation_steps 4 \
    --rla_stage residual_and_decoder \
    --d_video_feat 3318 \
    --d_audio_feat 6373 \
    --rla_hidden_video 256 \
    --rla_hidden_audio 256 \
    --rla_p_moddrop_video 0.10 \
    --rla_p_moddrop_audio 0.10 \
    --rla_video_temporal meanstd \
    --rla_video_norm none \
    --rla_audio_norm l2 \
    --rla_audio_temporal none \
    --rla_audio_alpha_init 3.0 \
    --rla_audio_use_ln \
    --rla_video_alpha_init 3.0 \
    --use_rla_video \
    --rla_video_use_ln \
    --format_prompt "" \
    --max_prompt_length 8096

  echo "Run finished for task: ${TASK}"
done

echo "All runs finished."