...
class AddQAOmniClassifierDataset(RLHFDataset):
    ...
    def __getitem__(self, item):
        ...
        # === QA rows: pass-through raw answer text for LM, skip label map ===
        # if the row is QA
        if dataset_name in self.qa_datasets:
            # Trainer will tokenize this string and align to the tail of input_ids
            row_dict["lm_labels"] = original_answer
            # CE won't be used for QA rows (trainer computes CE only on CLS rows),
            # but keep a valid tensor to avoid dtype issues downstream:
            row_dict["labels"] = torch.tensor(0, dtype=torch.long)
            return row_dict

        # preparation of classification (if the row is not QA)
        if dataset_name:
            full_label_key = f"{dataset_name}_{original_answer}"
        else:
            full_label_key = original_answer
        ...
        if self.label_map is not None:
            ...
            if label == 0 and full_label_key not in self.label_map:
                ...
                print(f"[WARN] Raw label: '{original_answer}', Dataset: '{dataset_name}'")
